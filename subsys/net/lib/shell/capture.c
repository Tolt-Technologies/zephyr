...
static void capture_cb(struct net_capture_info *info, void *user_data)
{
	...
	if (info->is_custom_handler) {
		/* Custom handler mode - simplified output */
		if (*count == 0) {
			PR("Mode: Custom Handler\n");
			PR("Capture iface   Enabled\n");
		}

		PR("%s              %s\n",
		   info->capture_iface ?
		   (char[4]){info->capture_iface ?
			     (net_if_get_by_iface(info->capture_iface) + '0') : '-',
			     0} : "all",
		   info->is_enabled ? "yes" : "no");
	} else {
		/* IPIP tunnel mode - original output */
...
static int cmd_net_capture(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (count == 0) {
#if defined(CONFIG_NET_CAPTURE_CUSTOM_HANDLER)
		PR_INFO("No custom handler registered\n");
#else
		PR_INFO("No capture devices configured\n");
#endif
	}
#else
	ARG_UNUSED(argc);
...
static int cmd_net_capture_enable(const struct shell *sh, size_t argc, char *argv[])
{
...
#if !defined(CONFIG_NET_CAPTURE_CUSTOM_HANDLER)
	if (capture_dev == NULL) {
		PR_WARNING("Capture not configured. Run 'net capture setup' first.\n");
		return -ENOEXEC;
	}
#endif
	...
	if (argv[arg] == NULL) {
#if defined(CONFIG_NET_CAPTURE_CUSTOM_HANDLER)
		/* In custom handler mode, NULL iface means capture all */
		iface = NULL;
		PR_INFO("Enabling capture on all interfaces\n");
#else
